...
\end{itemize}
	...
	\begin{equation}
		\label{eq:czasWW}
		T_{WC} = \sum\limits_{i=1}^N T_{WIi} = \sum\limits_{i=1}^N (T_{PZi} + T_{TRANS1i}) + \sum\limits_{j=1}^N ( + T_{AZj} + T_{CSj} + T_{POj} + T_TRANS2j}) + \sum\limits_{i=1}^N (T_{CMi}+T_{T_{AOi}) + 2NT_{DO}
	\end{equation}
	\begin{equation}
		\label{eq:czasPO}
